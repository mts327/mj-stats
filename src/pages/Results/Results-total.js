...
function setTotal(results) {
    let totalResults = [];
    results.forEach(result => {
        let index = totalResults.findIndex(({ id }) => id == result.id);
        if (index == -1) {
            let newStat = result;
            newStat.year = "通算";
            totalResults.push(newStat);
        }
        else {
            totalResults[index].first += result.first;
            totalResults[index].second += result.second;
            totalResults[index].third += result.third;
            totalResults[index].fourth += result.fourth;
            totalResults[index].pt += result.pt;
            totalResults[index].times += result.times;
        }
    })
...
export function ResultsTotal(props) {
